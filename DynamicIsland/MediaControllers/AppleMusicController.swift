...
class AppleMusicController: MediaControllerProtocol {
    ...
    // Temporary diagnostics: AppleScript failures were swallowed by try?, which
    // made "music not detected" undebuggable. Writes to /tmp/atoll-music-debug.log.
    static func musicDebugLog(_ message: String) {
        let line = "\(Date()) \(message)\n"
        let url = URL(fileURLWithPath: "/tmp/atoll-music-debug.log")
    ...
    private static func fetchAppleMusicPlaybackInfo() async -> AppleMusicPlaybackInfo? {
        ...
        do {
            descriptor = try await fetchPlaybackInfoDescriptor()
        } catch {
            musicDebugLog("fetch error: \(error)")
            return nil
        }
        guard let descriptor else {
            musicDebugLog("nil descriptor")
            return nil
        }
        guard descriptor.numberOfItems >= 9 else {
            musicDebugLog("short descriptor: items=\(descriptor.numberOfItems)")
            return nil
        }

        musicDebugLog("parsed: playing=\(descriptor.atIndex(1)?.booleanValue ?? false) title=\(descriptor.atIndex(2)?.stringValue ?? "?") pos=\(descriptor.atIndex(5)?.doubleValue ?? -1)")
