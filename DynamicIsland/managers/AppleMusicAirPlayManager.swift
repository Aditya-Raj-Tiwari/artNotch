...
final class AppleMusicAirPlayManager: ObservableObject {
    ...
    func toggleDevice(_ device: AirPlayDevice) async {
        ...
        let escapedName = appleScriptEscapedAirPlayName(device.name)
        let script = """
        tell application "Music"
            try
    ...
    private func commitVolume(volume: Int, deviceName: String, deviceID: String) async {
        let escapedName = appleScriptEscapedAirPlayName(deviceName)
    ...
    /// AirPlay device names are advertised by the device itself, so they are
    /// adjacent-attacker-controlled input. Backslash must be escaped first or a
    /// name ending in \ re-opens the AppleScript string literal.
    private func appleScriptEscapedAirPlayName(_ name: String) -> String {
        name
