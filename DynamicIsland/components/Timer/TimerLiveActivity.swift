...
struct TimerLiveActivity: View {
    ...
    private var countdownSection: some View {
        ...
        return VStack(spacing: 4) {
            // No numericText roll here: the closed notch band leaves only ~2pt of
            // headroom and the rolling digits get shaved by the notch clip shape
            // every tick. A plain swap also avoids animating the whole live
            // activity layout once per second.
            Text(timerManager.formattedRemainingTime())
                .font(.system(size: 13, weight: .semibold, design: .monospaced))
                .foregroundColor(timerManager.isOvertime ? .red : .white)
                .lineLimit(1)
                .fixedSize(horizontal: true, vertical: false)
                .animation(nil, value: timerManager.remainingTime)
